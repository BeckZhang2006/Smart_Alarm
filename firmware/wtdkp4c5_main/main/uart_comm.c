...
// 发送数据帧
static esp_err_t send_frame(uart_resp_type_t type, const uint8_t *data, uint16_t data_len)
{
    if (data_len > 128) {
...
esp_err_t uart_send_command(uart_cmd_type_t type, const uint8_t *data, uint16_t data_len)
{
    ...
    int ret = uart_write_bytes(UART_PORT_NUM, (const char *)frame, frame_len);
    if (ret != frame_len) {
        ESP_LOGE(TAG, "Failed to send command, ret=%d", ret);
        return ESP_FAIL;
    }
    
    ESP_LOGD(TAG, "Command sent: type=%02X, seq=%d, len=%d", type, seq_number-1, data_len);
    return ESP_OK;
}
...
BaseType_t uart_receive_response(uart_response_t *resp, TickType_t wait_time)
{
    ...
    if (rx_len > 0) {
        ...
        if (parsed > 0) {
            ...
            memcpy(resp->data, cmd.data, cmd.data_len);
            
            ESP_LOGD(TAG, "Response received: type=%02X, seq=%d, len=%d",
                     resp->type, resp->seq, resp->data_len);
            
            // 解析状态响应
            if (resp->type == RESP_STATUS && resp->data_len >= 6) {
                is_person_present = resp->data[0] != 0;
                detection_confidence = resp->data[1] / 255.0f;
                // 可以解析更多数据...
            }
